...
test.case("fail", assert => {
  assert(p.date).invalid_type(["1"]);
});

test.case("pass", assert => {
  assert(p.date).type<DateType>();

  const d = new Date();
  assert(p.date.parse(d)).equals(d).type<Date>();
});

test.case("loose", assert => {
  const d = new Date(1723718400000);

  assert(p.loose.date.parse(1723718400000)).equals(d);
});
...
test.case("default", assert => {
  ...
  [p.date.default(da), p.date.default(() => da)].forEach(d => {
