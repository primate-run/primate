    ...
    if (is.dict(v)) {
      ...
      for (const [op, op_val] of Object.entries(v)) {
        switch (op) {
          ...
          case "$ne": {
            if (value instanceof Date && op_val instanceof Date) {
              if (value.getTime() === op_val.getTime()) return false;
              break;
            }
            if (value instanceof URL && op_val instanceof URL) {
...
export default class MemoryDB extends DB {
  ...
  read(as: As, args: {
    ...
    if (args.count === true) {
      //assert.undefined(args.with);
      if (args.with !== undefined) throw E.count_with_incompatible();
