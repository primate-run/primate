...
func deser(request js.Value, key string) map[string]any {
...
func make_request(route t_request, request js.Value) any {
  go_request := Request{
    ...
    Body: NewBodyFromJS(request.Get("body")),
    Path:    deser(request, "path"),
    Query:   deser(request, "query"),
    Headers: deser(request, "headers"),
    Cookies: deser(request, "cookies"),
  }
